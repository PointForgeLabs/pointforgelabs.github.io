...
function sCross(cx,cy,ang,w,h2){var co=Math.cos(ang),si=Math.sin(ang),hw=w/2,hh=h2/2,tw=w*.25,th=h2*.25;var p=[{x:-tw,y:-hh},{x:tw,y:-hh},{x:tw,y:-th},{x:hw,y:-th},{x:hw,y:th},{x:tw,y:th},{x:tw,y:hh},{x:-tw,y:hh},{x:-tw,y:th},{x:-hw,y:th},{x:-hw,y:-th},{x:-tw,y:-th}].map(function(v){return{x:cx+v.x*co-v.y*si,y:cy+v.x*si+v.y*co};});return"M"+p[0].x.toFixed(1)+","+p[0].y.toFixed(1)+p.slice(1).map(function(v){return"L"+v.x.toFixed(1)+","+v.y.toFixed(1);}).join('')+"Z";}
function sArrow(cx,cy,ang,w,h2){var co=Math.cos(ang),si=Math.sin(ang),hw=w/2,hh=h2/2;var p=[{x:0,y:-hh},{x:hw,y:0},{x:hw*.3,y:0},{x:hw*.3,y:hh},{x:-hw*.3,y:hh},{x:-hw*.3,y:0},{x:-hw,y:0}].map(function(v){return{x:cx+v.x*co-v.y*si,y:cy+v.x*si+v.y*co};});return"M"+p[0].x.toFixed(1)+","+p[0].y.toFixed(1)+p.slice(1).map(function(v){return"L"+v.x.toFixed(1)+","+v.y.toFixed(1);}).join('')+"Z";}
function sSpiral2(cx,cy,ang,w){var r=Math.max(w,w)/2;var d='';for(var i=0;i<=40;i++){var t=i/40;var a2=ang+t*Math.PI*4;var ri=r*t;d+=(i===0?'M':'L')+(cx+Math.cos(a2)*ri).toFixed(1)+","+(cy+Math.sin(a2)*ri).toFixed(1);}return d;}
function sWave2(cx,cy,ang,w,h2){var co=Math.cos(ang),si=Math.sin(ang),hw=w/2;var d='';for(var i=0;i<=16;i++){var t=i/16;var px=-hw+t*w;var py=Math.sin(t*Math.PI*3)*h2*.4;d+=(i===0?'M':'L')+(cx+px*co-py*si).toFixed(1)+","+(cy+px*si+py*co).toFixed(1);}return d;}
function sHeart(cx,cy,ang,w,h2){var co=Math.cos(ang),si=Math.sin(ang),s=Math.max(w,h2)/2;var d='';for(var i=0;i<=30;i++){var t=(i/30)*Math.PI*2;var px=16*Math.pow(Math.sin(t),3);var py=-(13*Math.cos(t)-5*Math.cos(2*t)-2*Math.cos(3*t)-Math.cos(4*t));px=px/17*s;py=py/17*s;d+=(i===0?'M':'L')+(cx+px*co-py*si).toFixed(1)+","+(cy+px*si+py*co).toFixed(1);}return d+"Z";}
var SHAPES={rect:{n:"Rect",f:sRect},circle:{n:"Circle",f:sCirc},ellipse:{n:"Ellipse",f:sEllip},tri:{n:"Tri",f:sTri},hex:{n:"Hex",f:sHex},diamond:{n:"Diamond",f:sDia},line:{n:"Line",f:sLine},dot:{n:"Dot",f:sDot},star:{n:"Star",f:sStar},cross:{n:"Cross",f:sCross},arrow:{n:"Arrow",f:sArrow},spiral2:{n:"Spiral",f:sSpiral2},wave2:{n:"Wave",f:sWave2},heart:{n:"Heart",f:sHeart}};
